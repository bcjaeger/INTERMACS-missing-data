...
__Response__ We appreciate the reviewer's helpful suggestion. We reviewed potential mechanisms for missingness in all of the analysis variables and found some additional notes to include in our revision. On lines X-X of page Y, we write "Data on surgery time and CV pressure was recorded less frequently in the INTERMACS registry prior to 2014, and the  combination of missing CV pressure and surgery time is the most frequently occurring missing pattern in the current analysis (Figure 1). In addition, missing values for peripheral edema occurred more frequently in 2014-2015 due to FILL IN"
...
__Response__: We have included a table that lists pros and cons for each imputation strategy we analyzed. The table also gives a recommendation for these strategies based on data from the current analysis.
