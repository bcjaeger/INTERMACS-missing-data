...
rspec_unscaled <- round_spec() %>% 
  round_using_decimal(digits = 4)

inline_md_strat_unscaled <- tbl_md_strat %>%
  bind_rows(.id = 'outcome') %>% 
  mutate(tbv = table_glue("{est/100} ({lwr/100}, {upr/100})", 
                          rspec = rspec_unscaled)) %>% 
  split(f = list(.$outcome,
                 .$model,
                 .$md_strat,
                 .$additional_missing_pct)) %>% 
...
# inline results for abstract

inline_auc_rf_vs_mean <- 
  inline_md_strat_unscaled$dead.auc.cph.ranger_mi.0 %>% 
  bracket_insert_left('25th, 75th percentile: ')

prob_rf_mi_gt_mean <- 
  fig_md_strat_infer$auc$probs$`Multiple imputation` %>% 
  filter(m2 == 'ranger') %>% 
  pull(prob_gt_meanmode)

prob_rf_si_gt_mean <- 
  fig_md_strat_infer$auc$probs$`Single imputation` %>% 
  filter(m2 == 'ranger') %>% 
  pull(prob_gt_meanmode)
...
__Results__ Multiple imputation with random forests emerged as a robust strategy to impute missing values, increasing model concordance by `r inline_auc_rf_vs_mean` compared with imputation to the mean for mortality risk prediction using a downstream proportional hazards model. The posterior probability that single and multiple imputation using random forests would improve concordance versus mean imputation was `r prob_rf_si_gt_mean` and `r prob_rf_mi_gt_mean`, respectively.
